...
def refine_bbox_by_score(S, fg_mask, bbox, iters=60, step=4, shrink=0.94,
...
def canonical_landmarks_ibug68(size=256):
    ...
    for i, a in enumerate(ang):
        r = np.deg2rad(a)
        # 위쪽(관자놀이) 구간은 rx를 줄이고, 아래(턱)로 갈수록 rx를 키움
...
def clamp_points_to_mask(pts, fg_mask, margin=1):
    H, W = fg_mask.shape
...
def compute_face_scale_from_plane(canon_pts, lm_type, bbox, support_mask, canonical_size=256, fill=0.85):
    ...
    # 즉, w*rw*face_scale <= avail_w*fill  and  h*rh*face_scale <= avail_h*fill
...
def build_normal_rig_auto(base_image_path, save_dir="out", canonical_size=256,
                          landmarks_type="lite20", emotion="normal", intensity=0.6,
    ...
    bbox = pick_bbox(gray, S, fg_mask,
                     ratio_list=(1.2, 2.0, 3.0, 4.0),
                     k_sizes=24, min_scale=0.10, max_scale=0.60)
    # bbox = refine_bbox_by_score(S, fg_mask, bbox, iters=80, step=3, shrink=0.95, min_fg=0.6)
    ...
    support = build_support_mask(gray, maps["FG"], flat_thr=0.65, edge_thr=0.35)
    ...
    # 비등방 계수 산출
    aspect = estimate_aspect_from_mask(maps["FG"], fallback_aspect=1.2)  # >=1
...
# CLI
# -----------------------------
# [ image, out ] 제외 default로 실행 권장 
# image는 assets 폴더에 이미 있다는 가정
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", type=str, default="/workspace/dhe_project/generation/out_obj")
    ...
    parser.add_argument("--face_scale", type=float, default=1.1, help="기본 전체 스케일(등방, auto 미사용 시)")
    parser.add_argument("--stretch_alpha", type=float, default=0.5, help="가로/세로 비율 반영 강도(0~1)")
    parser.add_argument("--stretch_limit", type=float, default=1.2, help="가로 늘림 상한")
    ...
    build_normal_rig_auto(
        ...
        mode=("auto" if 1 else None), # auto 상시 사용
    )